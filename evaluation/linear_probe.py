    ...
    def linear_probing(self,  trainer: Trainer, pl_module: LightningModule, log: bool = True):
        if trainer.global_rank == 0:
            ...
            for downstream_data_mod, dataset in zip(self.downstream_data_modules, self.names):
                ...
                print_scores = {}
                for k, v in val_scores_.items():
                    full_key = f"val_{k}"
                    scores[full_key].append(v)
                    print_scores[full_key] = v
                for k, v in test_scores_.items():
                    full_key = f"test_{k}"
                    scores[full_key].append(v)
                    print_scores[full_key] = v
                print('Linear probe ({d}): {scores}'
                      .format(d=dataset, scores="  ".join(map(lambda k: "%s=%.3f"%(k, print_scores[k]), print_scores))))
            for k, v in list(scores.items()):
                if len(self.names) > 1 or self.include_dataset:
                    for i, dataset in enumerate(self.names):
                        scores["%s_%s"%(k, dataset)] = scores[k][i]
                scores[k] = np.mean(scores[k], axis=0)
...
def evaluate_linear_probe(
    ...
    val_acc = test_linear_probe(classifier, val_feats, val_labels, use_mean_accuracy,
...
def train_linear_probe(
    ...
    if multilabel:
        ...
        acc_meter = F1Score(task="multiclass", num_classes=NUM_C, average=average).to(device)
...
def test_linear_probe(
    ...
    def flatten_tensor(scores, dict, name="f1"):
        for idx in range(scores.shape[0]):
            dict[f"{name}.{idx}"] = scores[idx]
        return dict
    if logger is None:
        logger = logging.getLogger("Linear probing")
    # evaluate
    ...
    if multilabel:
        ...
        f1_score_per_class = f1_score_class(predictions, test_labels).cpu().numpy()
        f1_mean = float(f1_score(predictions, test_labels))
        assert (f1_mean - float(f1_score_per_class.mean())) < 1e-5
        ...
        scores_dict =  {"acc1": accuracy1, "f1_mean": f1_mean, "f1_weighted": f1_weighted, "acc1(subset)": acc1_sklearn}
        return flatten_tensor(f1_score_per_class, scores_dict)
    else:
        NUM_C = len(set(test_labels.cpu().numpy())) if num_classes is None else num_classes
        f1_score = F1Score(task="multiclass", num_classes=NUM_C, average=average).to(device)
        if use_sklearn:
